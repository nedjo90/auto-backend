...
export class RappelConsoRecallAdapter implements IRecallAdapter {
  ...
  async getRecalls(request: RecallRequest): Promise<RecallResponse> {
    ...
    const data = (await this.fetchWithRetry(url)) as unknown as RappelConsoApiResponse;
...
function buildWhereClause(request: RecallRequest): string {
  ...
  if (request.make) {
    parts.push(`nom_de_la_marque_du_produit like "${request.make}"`);
  }
  if (request.model) {
    parts.push(`noms_des_modeles_ou_references like "*${request.model}*"`);
  }
